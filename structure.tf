...
availability_zone = "${element(var.regions, 0)}"
map_public_ip_on_launch = true
tags =  {
   Name = "myvpc_public_subnet"
  }
}
resource "aws_subnet" "myvpc_public_subnet2" {
vpc_id = "${aws_vpc.myvpc.id}"
cidr_block = "${element(var.subnet_one_cidr, 1)}"
map_public_ip_on_launch = true
tags =  {
   Name = "myvpc_public_subnet2"
  }
}
resource "aws_subnet" "myvpc_public_subnet3" {
vpc_id = "${aws_vpc.myvpc.id}"
cidr_block = "${element(var.subnet_one_cidr, 2)}"
map_public_ip_on_launch = true
tags =  {
   Name = "myvpc_public_subnet3"
  }
}
...
availability_zone = "${element(var.regions, 1)}"
tags =  {
...
resource "aws_route_table_association" "myvpc_public_subnet_route_table2" {
subnet_id = "${aws_subnet.myvpc_public_subnet2.id}"
route_table_id = "${aws_route_table.myvpc_public_subnet_route_table.id}"
}
resource "aws_route_table_association" "myvpc_public_subnet_route_table3" {
subnet_id = "${aws_subnet.myvpc_public_subnet3.id}"
route_table_id = "${aws_route_table.myvpc_public_subnet_route_table.id}"
}
